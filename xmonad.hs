...
main = do
    ...
    xmonad $
        ...
        [ ((mod4Mask .|. shiftMask, xK_z), spawn "slock")
          -- (windows . S.greedyView $ (!! 0) myWorkspaces) >>
        ...
        , ((0, xK_Print), spawn "scrot")
        , ((mod4Mask .|. controlMask, xK_x), xmonadPrompt defaultXPConfig)] `additionalKeysP`
        myEmacsKeys `removeKeys`
...
myEmacsKeys = zip viewKeys (worker S.greedyView myWorkspaces) ++
  ...
  where
    ...
    (viewKeys, shiftKeys) = (,) <$> map (modWorkSpaceKey ++)
                                <*> map ((++) $ modWorkSpaceKey ++ "S-") $ baseWorkSpaceKeys
    ...
    miscKeys = [ ("M4-c e", spawn "emacsclient -c")
               ...
               , ("M4-g g", spawnHere
                              (myTerminal ++ " -e /home/emmanuel/Development/bin/ghc/bin/ghci"))
...
myManageHook = composeAll
                 ...
                 , className =? "google-chrome" --> doShift "netflix-chrome"
                 , className =? "chromium-browser" --> doShift "netflix-chrome"
                 , (className =? "Firefox" <&&> resource =? "Dialog") --> doFloat
                 , (className =? "Firefox" <&&> resource =? "Session Manager") --> doFloat
